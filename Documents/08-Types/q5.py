...
class Scope:
    ...
    def assign_type(self,n:str,t:type,):
        current = self
        func_name = None
        while current:

            # check param in func first
            if func_name and func_name in current.funcs:
                func = current.funcs[func_name]
                if n in func.param:
                    func.param[n] = t
                    return True
    ...
    def get_type(self,name:str,return_val=None):
        current = self
        func_name=None
        while current:
...
class StaticCheck(Visitor):
    ...
    def visitId(self,ctx,o:Scope):
        t = o.get_type(name=ctx.name,return_val=False)
        if t != False:
            return t
        raise UndeclaredIdentifier(ctx.name)